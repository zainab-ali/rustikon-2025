...
//
// Journey:
//  - Let's create a manager actor
//  - We tell it we've started.
...
async fn run(name: String) {
    ...
    let () = Delay::new(Duration::from_secs(100)).await;
    println!("Ran {}", name);
}
...
async fn start_unit(units: Vec<UnitDef>, mut orchestrator: mpsc::Sender<Message>, name: String) {
    ...
    let x = join_all(dependencies).await;
...
async fn orchestrator(
    ...
    while let Some(message) = inbox.next().await {
        match message {
            Message::Start { name, sender } => match state.get_mut(&name) {
                None => {
...
async fn manager(
    ...
    loop {
        ...
        match maybe_message {
            ...
            let service = start_unit(units.clone(), orchestrator.clone(), name);
